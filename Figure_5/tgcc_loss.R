...
tgcc_compare_with_naive <- function(
  data,
  lamseq,
  bandwidth = NULL,
  useNorm = TRUE,
  ...
  for (i in 1:k) {
    ...
    fit.time.naive <- fit.time.naive + (toc1 - tic1)[3]

    if(findLoss) {
      Losstrue[i] <- findloss(data, Theta[[i]], lamseq[i], parents, edgeWeights)
    }
  }
  ...
  for (i in 1:k) {
    ...
    # end
    toc2 <- proc.time()
    fit.time.tgcc <- fit.time.tgcc + (toc2 - tic2)[3]

    if (findLoss) {
      Losstgcc[i] = findloss(data, Theta2[[i]], lamseq[i], params$Parents, params$EdgeWeights)
    }
  ...
  list(
    ...
    Thetatrue = Theta,
    Thetatgcc = Theta2,
    runtimeNaive = fit.time.naive,
    runtimeTGCC = fit.time.tgcc
  )
}
